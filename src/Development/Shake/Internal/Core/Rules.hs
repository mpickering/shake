...
binarySummary :: (BinaryEx value) => BuiltinSummary key value
binarySummary _ value = return $! force $ SHA.showDigest $ SHA.sha256 $
    LBS.fromStrict $ encodeEx value
...
addBuiltinRuleEx :: (RuleResult key ~ value
                    , ShakeValue key
                    , BinaryEx key
                    , Typeable value
                    , NFData value
                    , Show value
                    ) => BuiltinLint key value -> BuiltinSummary key value -> BuiltinRun key value -> Rules ()
...
addBuiltinRuleInternal :: (RuleResult key ~ value
                          , ShakeValue key
                          , Typeable value
                          , NFData value
                          , Show value
                          ) => BinaryOp key -> BuiltinLint key value -> BuiltinSummary key value -> BuiltinRun key value -> Rules ()

addBuiltinRuleInternal binary lint summary (run :: BuiltinRun key value) = do
